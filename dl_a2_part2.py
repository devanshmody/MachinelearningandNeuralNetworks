...
#design_model function is used to create the model from scratch
...
def scratch_model(x_train,y_train,x_test,y_test,nclas,epoch,batch,value,w):
  ...
  for i in range(0,3):
    ...
    #early stopping method is used to montior the loss if there are no significant reductions in loss then it waits for max five iterations and then halts the training
    ...
    plotgraph(history,value,"from scratch")
    ...
    st=time.time()
    #evalute the model
    (loss, accuracy) = model.evaluate(x_test,y_test, batch_size=batch,verbose=1,workers=50,
                        use_multiprocessing=True)
    #store testing time
    tt+=time.time()-st
  ...
  print("Model from scratch Training accuracy for {} is : {}%".format(value,avg_tr*100))
  ...
  print("Model from scratch Testing accuracy for {} is : {}%".format(value,avg_tt*100))
    

#this function is used for transfer learning and without transfer learning
def design_densenet(xtrain,xtest,Y_train,Y_test,value,epoch,batch,nclas,val):
  #start time training
  st=time.time()
  #this is called by the callback in model.fit method to schedule the decay of learning rates
  lr_scheduler = LearningRateScheduler(decay_schedule)
  ...
  #early stopping method is used to montior the loss if there are no significant reductions in loss then it waits for max five iterations and then halts the training
  es = EarlyStopping(monitor='loss',patience=5)
  #input shape for Densenet201 model
  image_input = Input(shape=(224, 224, 3))
  #using pretained weights from imagnet for transfer learning if val==1 and val==0 without transfer learning
  if val==1:
    model = DenseNet201(input_tensor=image_input,include_top=True, weights='imagenet')  
    print("transfer learning")
  if val==0:
    model = DenseNet201(input_tensor=image_input,include_top=True, weights=None)
    print("without transfer learning")

  last_layer = model.get_layer('avg_pool').output
  out = Dense(nclas, activation='softmax', name='output')(last_layer)
  #creating my model with  my own classifier layer at last and keeping all above layers to their default value 
  densenet_model = Model(image_input, out)
  #creating my model with  my own classifier layer at last and keeping all above layers to their default value 
  ad = Adam(lr=0.001,beta_1=0.9, beta_2=0.999, epsilon=1e-08)
  ...
  history = densenet_model.fit(xtrain,Y_train,
                                       epochs=epoch,
                                       batch_size=batch,
                                       shuffle=True,
                                       verbose=1,
  ...
  tr=time.time()-st
  if val==1:
    #plot accuracy and loss curve for with transfer learning
    plotgraph(history,value,"with Transfer learning")
  if val==0:
    #plot accuracy and loss curve for without transfer learning
    plotgraph(history,value,"without Transfer learning")
  tr_acc=history.history["accuracy"][-1:]
  st=time.time()
  #evalute the model
  (loss, accuracy) = densenet_model.evaluate(xtest, Y_test, batch_size=batch,verbose=1,workers=50,
                        use_multiprocessing=True)
  tt=time.time()-st
  #free memory
  del model,history,densenet_model
  return tr,tt,tr_acc,accuracy

#this function call design densenet for with  and without transfer learning for 15scene and caltech101 dataset
def densenet(xtrain,xtest,Y_train,Y_test,value,epoch,batch,nclas):
  #list to store the training and testing accuracy
  tr_acc=[]
  tt_acc=[]
  #tr and tt is used store training and testing time 
  tr=0.0
  tt=0.0
  #get average of three accuracy for transfer learninng using imagenet pretrained weights
  for i in range(0,3):  
    tr_time,tt_time,train_acc,test_acc=design_densenet(xtrain,xtest,Y_train,Y_test,value,epoch,batch,nclas,1)
    tr+=tr_time
    tt+=tt_time
  ...
  #calculate the average of testing accuracy of 3 runs
  avg_tt=tf.add_n(tt_acc)/3
  avg_tt=np.float(avg_tt)
  print("Testing accuracy with transfer learning for {} is : {}%".format(value,avg_tt*100))

  #without transfer learning
  tr_time,tt_time,train_acc,test_acc=design_densenet(xtrain,xtest,Y_train,Y_test,value,epoch,batch,nclas,0)
  acc=np.float(train_acc[-1])
  print("Training time without transfer learning for {} is {}".format(value,tr_time))
  print("Training accuracy without transfer learning for {} is : {}%".format(value,acc*100))
  print("Testing time without transfer learning for {} is {}".format(value,tt_time))
  print("Testing accuracy without transfer learning for {} is : {}%".format(value,test_acc*100))

def main():
  #hyperparameter
  epoch=60
  batch=32
  nclas=15 #for 15scene
  size=224
  #call scratch model,denset model with and without transfer learning for 15scene dataset
  sx_train,sy_train,sx_test,sy_test=get_data('/content/drive/MyDrive/15-Scene',0.6655518394648829,nclas,size)
  print("length of training data is {} and length of testing data is {}".format(len(sx_train),len(sx_test)))
  #call with and without trasnfer learning densenet model
  densenet(sx_train,sx_test,sy_train,sy_test,"15Scene",epoch,batch,nclas)  
  #call model from scratch
  scratch_model(sx_train,sy_train,sx_test,sy_test,nclas,epoch,batch,"15-scene",size)
  #free memory
  ...
  #call with and without trasnfer learning densenet model
  densenet(cx_train,cx_test,cy_train,cy_test,"caltech101",epoch,batch,nclas)
  #call model from scratch
  scratch_model(cx_train,cy_train,cx_test,cy_test,nclas,epoch,batch,"caltech101",size)
  #free memory
